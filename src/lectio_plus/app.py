...
def create_app(*, default_date: str | None = None) -> Flask:
    ...
    def index() -> str:
        ...
        return (
            ...
            "#overlay{position:fixed;inset:0;background:radial-gradient(ellipse at top,#fff 0%,rgba(255,255,255,.92) 45%,rgba(255,255,255,.9) 100%);display:none;align-items:center;justify-content:center;}"
            "#overlay .stage{position:absolute;inset:0;overflow:hidden}#overlay .msg{position:relative;z-index:2;background:rgba(255,255,255,0.85);padding:10px 14px;border-radius:8px;border:1px solid #ddd;color:#333;font-weight:600;box-shadow:0 2px 10px rgba(0,0,0,.08)}"
            "/* Psychedelic geometric flow */"
            "#overlay .geo{position:absolute;inset:-20%;filter:blur(18px);opacity:.85}"
            "#overlay .geo::before,#overlay .geo::after{content:'';position:absolute;inset:0;background:conic-gradient(from 0deg, #ff0080, #ff8c00, #ffd500, #00e676, #00b0ff, #6a00ff, #ff0080);mix-blend-mode:screen;animation:spin 14s linear infinite}"
            "#overlay .geo::after{animation-duration:21s;animation-direction:reverse;opacity:.7}"
            "#overlay .blobs{position:absolute;inset:-10%}#overlay .blob{position:absolute;width:48vmax;height:48vmax;border-radius:40% 60% 70% 30% / 40% 40% 60% 60%;background:radial-gradient(circle at 30% 30%, rgba(255,0,128,.45), rgba(255,0,128,0) 60%), radial-gradient(circle at 70% 70%, rgba(0,176,255,.45), rgba(0,176,255,0) 60%);animation:drift 16s ease-in-out infinite alternate}"
            "#overlay .blob.b2{left:40%;top:10%;transform:scale(0.9);animation-duration:19s}#overlay .blob.b3{left:10%;top:45%;transform:scale(1.1);animation-duration:22s}"
            "#overlay .grid{position:absolute;inset:-5%;background:repeating-conic-gradient(from 0deg, rgba(0,0,0,.05) 0 10deg, transparent 10deg 20deg);mask:radial-gradient(circle at 50% 50%, rgba(0,0,0,1), rgba(0,0,0,0) 65%);animation:spin 60s linear infinite}"
            "@keyframes spin{to{transform:rotate(1turn)}}@keyframes drift{0%{transform:translate(-10%,-5%) scale(1)}100%{transform:translate(10%,5%) scale(1.15)}}@media (prefers-reduced-motion:reduce){#overlay .geo,#overlay .blobs,#overlay .grid{animation:none}#overlay .blob{display:none}}"
